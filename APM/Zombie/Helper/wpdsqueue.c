...
struct WPDSQueue {
     ...
     * When `length` = 0, wp_ds_queue_get will block.
     * When `length` = `capacity`, wp_ds_queue_put will block. */
    uint32_t length;
...
wp_ds_queue_put_pop_first_item_if_need(struct WPDSQueue *queue, void *item)
{
    ...
    if (queue->capacity > 0 && queue->length == queue->capacity) {
